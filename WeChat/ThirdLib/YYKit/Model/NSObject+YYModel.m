...
- (BOOL)modelIsEqualToModel:(id)model {
    if (self == model) return YES;
    if (![model isMemberOfClass:self.class]) return NO;
    _YYModelMeta *modelMeta = [_YYModelMeta metaWithClass:self.class];
    if (modelMeta->_nsType) return [self isEqual:model];
    ...
    for (_YYModelPropertyMeta *propertyMeta in modelMeta->_allPropertyMetas) {
        if (!propertyMeta->_isKVCCompatible) continue;
        NSString *propertyMetaGetter = NSStringFromSelector(propertyMeta->_getter);
        if ([propertyMetaGetter isEqualToString:@"classId"] || [propertyMetaGetter isEqualToString:@"className"] || [propertyMetaGetter isEqualToString:@"inviterInfoModel"] || [propertyMetaGetter isEqualToString:@"groupName"]) continue;
        id this = [self valueForKey:propertyMetaGetter];
        id that = [model valueForKey:propertyMetaGetter];
        if (this == that) continue;
        if (this == nil || that == nil) return NO;
        if (![this isEqual:that]) return NO;
    }
